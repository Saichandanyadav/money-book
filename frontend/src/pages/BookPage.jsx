...
const BookPage = () => {
    ...
    const handlePendingFilterChange = (e) => {
        const newFilter = { ...pendingFilter, [e.target.name]: e.target.value };
    ...
    const handleClearFilters = () => {
        const newFilter = { modeOfPayment: "", status: "", date: "" };
        setFilter(newFilter);
        setPendingFilter(newFilter);
        setCurrentPage(1);
    ...
    const paymentFormOptions = [
        ...paymentMethods.map(mode => ({ label: mode, value: mode })),
        { label: "-- Add New Mode --", value: OTHER_OPTION },
    ];
    ...
    return (
        ...
        <div className="p-4 sm:p-6 max-w-7xl mx-auto bg-gradient-to-b from-white to-sky-50 min-h-screen relative pb-20 sm:pb-4">
            ...
            {activeTab === "overview" && (
                <OverviewSection overviewData={overviewData} symbol={symbol} />
            )}
            ...
            {activeTab === "stats" && (
                <StatCharts transactions={transactions} book={book} symbol={symbol} />
            )}
            
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-sky-200 shadow-xl sm:hidden">
                <div className="flex justify-around gap-3">
                    <button 
                        onClick={() => { setTransactionType("credit"); setShowModal(true); }} 
                        disabled={!book.isActive} 
                        className="flex-1 px-3 py-2 bg-white border border-green-500 text-green-600 rounded-lg text-sm font-semibold hover:bg-green-50 disabled:opacity-50 transition duration-150 shadow-md"
                    >
                        Add Money
                    </button>
                    <button 
                        onClick={() => { setTransactionType("debit"); setShowModal(true); }} 
                        disabled={!book.isActive} 
                        className="flex-1 px-3 py-2 bg-white border border-red-500 text-red-600 rounded-lg text-sm font-semibold hover:bg-red-50 disabled:opacity-50 transition duration-150 shadow-md"
                    >
                        Send Money
                    </button>
                </div>
            ...
            {confirmModal.visible && (
                <Modal 
                    title={`Confirm ${confirmModal.type === "toggleStatus" ? "Status Change" : "Deletion"}`}
                    onClose={() => setConfirmModal({ type: "", visible: false })}
                    onSubmit={confirmModal.type === "toggleStatus" ? handleToggleStatus : handleDelete}
                    submitText={confirmModal.type === "toggleStatus" ? (book.isActive ? "Deactivate" : "Activate") : "Delete"}
                    submitColor={confirmModal.type === "toggleStatus" ? (book.isActive ? "yellow" : "blue") : "red"}
                >
            ...
            {showAddPaymentModal && (
                <AddPaymentModal
                    onClose={() => { setShowAddPaymentModal(false); setShowModal(true); }}
